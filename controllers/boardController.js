...
const getList = async (req, res) => {
    ...
    try {
        const { boards, totalPages, currentPage } 
            = await boardService.getBoardByCategory(category, page);
        res.render('board/list', { boards, totalPages, currentPage, category });
    } catch (error) {
        next(error);
    }
}
...
const getBoardInfo = async (req, res) => {
    ...
    try {
        const {comments, totalCommentPages, currentCommentPage} 
            = await commentService.getCommentByBoardId(boardId, commentPage);
        //특정 게시글에 단 댓글들 가져오기
        res.render('board/info', 
            { board, comments, totalCommentPages, currentCommentPage});
    } catch (error) {
        next(error);
    }
}
...
const getBoardModify = async (req, res, next) => {
    try {
        ...
        res.render('board/modify', { board: modifyBoard });
    } catch (error) {
        next(error);
    }
    if (!req.isAuthenticated()) {
...
const getBoardDelete = (req, res) => {
    ...
    res.render('board/delete', { boardId: req.params.boardId });
}
...
module.exports = {
    getWrite, postWrite, getList, getBoardInfo, getBoardModify, postBoardModify,
    getBoardDelete, postBoardDelete, getMainBoards
}